...
var secretsClient = new import_client_secrets_manager.SecretsManagerClient({ region });
var s3Client = new import_client_s3.S3Client({ region });
var cloudfrontClient = new import_client_cloudfront.CloudFrontClient({ region });
var TOPIC_SCHEDULE = {
  0: {
    // Sunday
...
async function getSecret(secretArn) {
  const command = new import_client_secrets_manager.GetSecretValueCommand({ SecretId: secretArn });
  const response = await secretsClient.send(command);
  return response.SecretString || "";
}
function generateSlug(title) {
  return title.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").substring(0, 60).trim();
}
function getTodayConfig() {
  const dayOfWeek = (/* @__PURE__ */ new Date()).getDay();
  return TOPIC_SCHEDULE[dayOfWeek];
}
async function generateBlogContent(openai, config) {
...
function generateHTML(post) {
  ...
  const sectionsHTML = post.sections.map((section, index) => {
    const paragraphs = section.content.split("\n\n").map(
      (p2) => `<p class="text-gray-600 mb-6 leading-relaxed">${escapeHtml(p2)}</p>`
    ).join("\n");
    const sectionHTML = `
      <div class="mb-10">
  ...
  <footer class="py-12 px-6 bg-gray-900 border-t border-gray-800">
    <div class="max-w-7xl mx-auto">
      <div class="flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <p class="text-gray-500 text-sm">&copy; ${(/* @__PURE__ */ new Date()).getFullYear()} Haus Edge Capital.</p>
      </div>
...
async function getExistingGeneratedPosts(bucket) {
  try {
...
async function getStaticBlogPosts(bucket) {
  try {
    const command = new import_client_s3.GetObjectCommand({
      Bucket: bucket,
...
async function updateSitemap(bucket, staticPosts, generatedPosts) {
...
async function invalidateCloudFront(distributionId, paths) {
  const command = new import_client_cloudfront.CreateInvalidationCommand({
...
var handler = async (event) => {
  ...
  try {
    ...
    const openai = new openai_default({ apiKey: openaiApiKey });
    ...
    if (existingGeneratedPosts.some((p2) => p2.slug === blogPost.slug)) {
